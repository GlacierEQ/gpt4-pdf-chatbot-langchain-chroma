...
`CHUNK_SIZE` and `CHUNK_OVERLAP` control how PDF text is split before embedding.
Adjust these values to tune retrieval granularity.
...
3. Run the script `npm run ingest` to ingest and embed your docs. You may pass
   ...
   Chroma instance. Text splitting behavior can be tuned with `CHUNK_SIZE` and
   `CHUNK_OVERLAP`. If you run into errors, troubleshoot below.
